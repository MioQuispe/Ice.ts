...
import { HashMap, List, Logger, LogLevel, Layer, Cause, Option } from "effect"
	...
	queueMicrotask(() => {
		try {
			...
			while (shared.queue.length > 0 && processed < 8) {
				...
				switch (ev.kind) {
					case "log": {
						if (ev.method === "info") clog.message(ev.line)
					...
					case "spinner_stop": {
						if (shared.spinner) {
							try {
								shared.spinner.stop(ev.msg, ev.code)
							} catch {}
							shared.spinner = null
						}
						break
		...
		({
			...
			if (message.length > 0) {
				...
				if (typeof firstMaybeString === "string") {
					firstLine +=
						" " + color(firstMaybeString, colors.bold, colors.cyan)
					messageIndex++
				}
			}
			...
			for (; messageIndex < message.length; messageIndex++) {
				...
				const line =
					typeof m === "string" ? m : Inspectable.stringifyCircular(m)
				emit(method, line)
			}
